...
/********************************************************************
	...
	Created:	2018 ~ 2023
	Filename: 	WebSocketRequest.cs
	Author:		DaiLi.Ou

	Descriptions: Abstract class of Web Socket Request.
*********************************************************************/
namespace Core.Network
{
    public class WebSocketRequest : IConnectorWebSocket
    {
        ...
        public void Connect()
        {
            ...
            lock (locker)
            {
                ...
                websocketConnector = new WebSocketConnector(host, port);
                websocketConnector.OnConnect += OnConnect;
                websocketConnector.OnClose += OnClose;
                websocketConnector.OnError += OnError;
                websocketConnector.OnMessage += OnMessage;
        ...
        public IEnumerator StartServer()
        {
            while (true)
            {
                ...
                while (queue.Count > 0)
                {
                    if (websocketConnector != null && websocketConnector.CurrentStatus == WebSocketConnector.Status.Establish)
                    {
                        websocketConnector.Send(queue.Dequeue());
                    }
                    else { break; }
                }
