...
namespace Core.Network
{
    ...
    public sealed class HttpWebRequestEntity
    {
        ...
        public HttpWebRequestEntity SetAccept(string str)
        {
            accept = str;
            return this;
        }
        ...
        {
            //return null;
            var webRequest = (System.Net.HttpWebRequest)System.Net.WebRequest.Create(uri);//new System.Net.HttpWebRequest();//new System.Net.HttpWebRequest(new Uri(uri));
            var requestResponse = new HttpWebRequestResponse(webRequest);
            webRequest.Method = method.ToString().ToUpper();

            //webRequest.KeepAxlive = false;
            ...
            if (requestBytes != null)
            {
                if (method != Restfuls.Delete)
                {
                    ...
                    if (!string.IsNullOrEmpty(accept)) webRequest.Accept = accept;
