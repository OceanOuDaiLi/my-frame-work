...
public sealed class SpawnPool : MonoBehaviour, IList<Transform>
{
    ...
    void Update()
    {
        ...
        {
            foreach (PrefabPool p in _prefabPools)
            {
                foreach (Transform t in p.spawned)
                {
                    if (t == null)
                    {
                        Debug.LogErrorFormat("SpawnPool: prefab '{0}' with null spawned transform", p.prefab.name);
                    }
                }
            }
        }
    ...
    public Transform Spawn(Transform prefab, Vector3 pos, Quaternion rot)
    {
        ...
        foreach (PrefabPool prefabPool in this._prefabPools)
        {
            ...
            if (prefabPool.prefabGO == prefab.gameObject)
            {
                ...
                if (inst.parent != this.group) inst.SetParent(this.group);
        ...
        inst.SetParent(this.group);// Add to this parent group
        // New instances are active and must be added to the internal list 
        this._spawned.Add(inst);
    ...
    private IEnumerator ListenForEmitDespawn(ParticleSystem emitter)
    {
        ...
        yield return Yielders.GetWaitForSeconds(emitter.main.startDelayMultiplier + 0.25f); //new WaitForSeconds(emitter.startDelay + 0.25f);  command by daili.ou 2023/07/04
...
public class PrefabPool
{
    ...
    internal Transform SpawnInstance(Vector3 pos, Quaternion rot)
    {
        ...
        else
        {
            ...
            inst = null;
            while (inst == null && this.despawned.Count > 0)
            {
                ...
                if (inst == null)
                {
                    var msg = "Make sure you didn't delete a despawned instance directly. (" + prefab.name + ")";
                    throw new MissingReferenceException(msg);
                }
#endif
            }

            if (inst == null && this.despawned.Count == 0) inst = this.SpawnNew(pos, rot);
    ...
    internal Transform SpawnNew(Vector3 pos, Quaternion rot)
    {
        ...
        inst.SetParent(this.spawnPool.group);
