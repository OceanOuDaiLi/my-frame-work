...
namespace Core.Resources
{
    ...
    public sealed class AssetBundleLoader : IAssetBundle
    {
        ...
        public AssetBundle LoadAssetBundle(string path)
        {
            ...
            var assetTarget = LoadAssetBundle(App.Env.AssetPath, (relPath + Path.AltDirectorySeparatorChar + objName).Trim('/'));
            return assetTarget;
        }
        ...
        {
            return App.Instance.StartCoroutine(LoadBundleAsyncIEnumerator(path, callback));
        }
        ...
        public Object LoadAsset(string path, System.Type type)
        {
            LoadManifest();
            string relPath, objName;
            ParsePath(path, out relPath, out objName);

            var assetTarget = LoadAssetBundle(App.Env.AssetPath, relPath);
            return assetTarget == null ? null : assetTarget.LoadAsset(objName, type);
        }
        ...
        /// <param name="path">加载路径</param>
        /// <returns></returns>
        public Object[] LoadAssetAll(string path, System.Type type)
        {
            LoadManifest();
            string relPath, objName;
            ParsePath(path, out relPath, out objName);

            var assetTarget = LoadAssetBundle(App.Env.AssetPath, relPath + Path.AltDirectorySeparatorChar + Path.GetFileNameWithoutExtension(objName));
            return assetTarget == null ? null : assetTarget.LoadAllAssets(type);
        }
        ...
        /// <returns>协程</returns>
        public UnityEngine.Coroutine LoadAssetAsync(string path, System.Type type, System.Action<Object> callback)
        {
            return App.Instance.StartCoroutine(LoadAssetAsyncIEnumerator(path, type, callback));
        }
        ...
        /// <param name="callback">回调</param>
        /// <returns>协程</returns>
        public UnityEngine.Coroutine LoadAssetAllAsync(string path, System.Type type, System.Action<Object[]> callback)
        {
            return App.Instance.StartCoroutine(LoadAssetAllAsyncIEnumerator(path, type, callback));
        }
        ...
        private IEnumerator LoadBundleAsyncIEnumerator(string path, System.Action<AssetBundle> callback)
        {
            ...
            yield return LoadAssetBundleAsync(App.Env.AssetPath, relPath, (ab) =>
            {
                assetTarget = ab;
            });
        ...
        /// <returns>迭代器</returns>
        private IEnumerator LoadAssetAsyncIEnumerator(string path, System.Type type, System.Action<Object> callback)
        {
            LoadManifest();
            string relPath, objName;
            ParsePath(path, out relPath, out objName);
            ...
            var targetAssetRequest = assetTarget.LoadAssetAsync(objName, type);
        ...
        /// <param name="callback">回调函数</param>
        /// <returns>迭代器</returns>
        private IEnumerator LoadAssetAllAsyncIEnumerator(string path, System.Type type, System.Action<Object[]> callback)
        {
            LoadManifest();
            string relPath, objName;
            ParsePath(path, out relPath, out objName);
            ...
            //yield return LoadAssetBundleAsync(App.Env.AssetPath, relPath + System.IO.Path.PathSeparator + System.IO.Path.GetFileNameWithoutExtension(objName), (ab) =>
            //{
            //    assetTarget = ab;
            //});
            ...
            var targetAssetRequest = assetTarget.LoadAllAssetsAsync(type);
            yield return targetAssetRequest;
