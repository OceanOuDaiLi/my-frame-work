...
namespace ET
{
	...
	public class KChannel : AChannel
	{
		...
		public KChannel(long id, uint localConn, Socket socket, IPEndPoint remoteEndPoint, KService kService)
		{
			...
			EDebug.Log($"channel create: {this.Id} {this.LocalConn} {remoteEndPoint} {this.ChannelType}");
		...
		public KChannel(long id, uint localConn, uint remoteConn, Socket socket, IPEndPoint remoteEndPoint, KService kService)
		{
			...
			EDebug.Log($"channel create: {this.Id} {localConn} {remoteConn} {remoteEndPoint} {this.ChannelType}");
		...
		public override void Dispose()
		{
			...
			EDebug.Log($"channel dispose: {this.Id} {localConn} {remoteConn}");
		...
		public void HandleConnnect()
		{
			...
			EDebug.Log($"channel connected: {this.Id} {this.LocalConn} {this.RemoteConn} {this.RemoteAddress}");
		...
		private void Connect()
		{
			try
			{
				...
				EDebug.Log($"kchannel connect {this.Id} {this.LocalConn} {this.RemoteConn} {this.RealAddress} {this.socket.LocalEndPoint}");
		...
		public void Update()
		{
			...
			if (!this.IsConnected)
			{
				...
				{
					EDebug.LogError($"kChannel connect timeout: {this.Id} {this.RemoteConn} {timeNow} {this.CreateTime} {this.ChannelType} {this.RemoteAddress}");
		...
		public void HandleRecv(byte[] date, int offset, int length)
		{
			...
			while (true)
			{
				...
				if (this.needReadSplitCount > 0) // 说明消息分片了
				{
					...
					if (n != count)
					{
						EDebug.LogError($"kchannel read error1: {this.LocalConn} {this.RemoteConn}");
					...
					{
						EDebug.LogError($"kchannel read error2: {this.LocalConn} {this.RemoteConn}");
				...
				else
				{
					...
					if (n == 8)
					{
						...
						if (headInt == 0)
						{
							...
							{
								EDebug.LogError($"kchannel read error3: {this.needReadSplitCount} {this.LocalConn} {this.RemoteConn}");
		...
		public void Send(long actorId, MemoryStream stream)
		{
			if (this.kcp != IntPtr.Zero)
			{
				...
				if (n > maxWaitSize)
				{
					EDebug.LogError($"kcp wait snd too large: {n}: {this.Id} {this.LocalConn} {this.RemoteConn}");
