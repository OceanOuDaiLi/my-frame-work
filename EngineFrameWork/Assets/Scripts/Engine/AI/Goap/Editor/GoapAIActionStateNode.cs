...
namespace Goap.AI
{
    public class GoapAIActionStateNode : GoapAIBaseNode
    {
        ...
        public GoapAIActionStateNode(Vector2 aPosition, float aWidth, float aHeight,
        ...
        public void BindData(string aTitle, GoapAIPlanner aPlanner, GoapAICondition aCur, GoapAICondition aPre)
        {
            title = string.Concat("▶ ", aTitle);

            _items = new List<Item>();
            bool v;
            for (int i = 0, n = GoapAIPlanner.MAX_ATOMS; i < n; i++)
            {
                if (aCur.GetMask(i))
                {
                    v = aCur.GetValue(i);
