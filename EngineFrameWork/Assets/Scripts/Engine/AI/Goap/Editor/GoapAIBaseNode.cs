...
namespace Goap.AI
{
    public abstract class GoapAIBaseNode
    {
        ...
        public List<GoapAIBaseNode> links;
        ...
        protected Color _color = GoapAIEditorStyle.CardWhite;
        ...
        public GoapAIBaseNode(Vector2 aPosition, float aWidth, float aHeight,
        ...
        public virtual bool ProcessEvents(Event aEvent, GoapAIWorkbench aWorkbench)
        {
            bool result = false;
            switch (aEvent.type)
            {
        ...
        {
            var pos = Position;
            float ang = GoapMath.AngleDeg(pos, aToPosition);
            return new Vector2(
        ...
        public Vector2 GetOutputPoint(GoapAIBaseNode aToNode)
        {
            var pos = Position;
            pos.x += rect.width * 0.5f;
            pos.y += rect.height * 0.5f;
            float ang = GoapMath.AngleDeg(pos, aToNode.Position);
            return new Vector2(
                pos.x + 10.0f * Mathf.Cos((ang + 90.0f) * Mathf.Deg2Rad),
                pos.y + 10.0f * Mathf.Sin((ang + 90.0f) * Mathf.Deg2Rad)
            );
        ...
        public Vector2 GetInputPoint(GoapAIBaseNode aFromNode)
        {
            var pos = Position;
            pos.x += rect.width * 0.5f;
            pos.y += rect.height * 0.5f;
            float ang = GoapMath.AngleDeg(pos, aFromNode.Position);
            return new Vector2(
                pos.x - 10.0f * Mathf.Cos((ang + 90.0f) * Mathf.Deg2Rad),
                pos.y - 10.0f * Mathf.Sin((ang + 90.0f) * Mathf.Deg2Rad)
            );
