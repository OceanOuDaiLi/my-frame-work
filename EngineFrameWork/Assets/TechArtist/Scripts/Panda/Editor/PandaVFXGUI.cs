...
/// <summary>
/// This is customer Shader GUI FrameWork.
/// Source from Panda VFX 2.0.Recommand from Effects' Artist
/// ReWrite by daili.ou 2023/08/09.
/// </summary>
...
public class PandaVFXGUI : ShaderGUI
{
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] props)
    {
        ...
        if (mainTex_Sampler.textureValue != null || material.GetFloat("_ScreenAsMain") == 1)
        {
            ...
            if (AddTex_Sampler.textureValue != null)
            {
                ...
                {
                    EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                    _AddTex = Foldout(_AddTex, "附加贴图(AddTexture)");
            ...
            else
            {
                ...
                {
                    EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                    _AddTex = Foldout2(_AddTex, "附加贴图(AddTexture)");
    ...
    public void GUI_Maintextures(Material material)
    {
        ...
        if (material.GetFloat("_ScreenAsMain") == 0)
        ...
        if (material.GetFloat("_ScreenAsMain") == 1)
        {
            m_MaterialEditor.ShaderProperty(TinColor, "主颜色");

            m_MaterialEditor.ShaderProperty(BackFaceColor, "背面叠加色");
        ...
        if (mainTex_Sampler.textureValue != null || material.GetFloat("_ScreenAsMain") == 1)
        {
...

            {
                ...
                EditorGUILayout.PrefixLabel("自定义数据");
                ...
                if (material.GetFloat("_CustomdataMainTexUV") == 1)
                {
                    ...
                    EditorGUILayout.PrefixLabel("影响附加图");
                    ...
                    m_MaterialEditor.ShaderProperty(CAddTexUVT, "保持同速偏移");
                ...
                if (_tips == true)
                {
                    ...
                    style.fontSize = 10;
                    style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
                    GUILayout.Label("*开启自定义数据控制贴图UVOffset，制作贴图一次性流动使用，开启自定义数据时请先添加uv2，再添加custom1.xyzw,再添加custom2.xyzw，然后custom1.x控制U方向Offset，custom1.y控制V方向Offset", style);
                    EditorGUILayout.EndVertical();
                }
                GUILayout.Space(5);
            ...
            EditorGUILayout.PrefixLabel("是否开启附加贴图");
            ...
            if (_tips == true)
            {
                ...
                style.fontSize = 10;
                style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
                GUILayout.Label("*开启后下面会出现附加图页签，附加图可以用来增加纹理细节", style);
                EditorGUILayout.EndVertical();
            }
...

    {
...

        {
            ...
            if (_tips == true)
            {
                ...
                style.fontSize = 10;
                style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
                GUILayout.Label("*开启自定义数据控制贴图UVOffset，制作贴图一次性流动使用，开启自定义数据时请先添加uv2，再添加custom1.xyzw,再添加custom2.xyzw，然后custom2.x控制U方向Offset，custom2.y控制V方向Offset", style);
                EditorGUILayout.EndVertical();
            }
...

    {
        ...
        if (Dissolve_Tex.textureValue != null)
        {
            ...
            EditorGUILayout.PrefixLabel("开启附加溶解图");
            ...
            if (_tips == true)
            {
                ...
                style.fontSize = 10;
                style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
                GUILayout.Label("*开启自定义数据控制溶解程度，开启自定义数据时请先添加uv2，再添加custom1.xyzw,再添加custom2.xyzw，然后custom1.z控制溶解程度，开启自定义数据后，溶解系数将会失效，并隐藏", style);
                EditorGUILayout.EndVertical();
            }
            ...
            if (material.GetFloat("_CustomdataDis") == 1)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.PrefixLabel("开启拖尾溶解模式");
            ...
            if (material.GetFloat("_soft_sting") == 0)
            {
                ...
                m_MaterialEditor.ShaderProperty(Dissolve_Soft, "溶解软硬程度");
        ...
        if (Distort_Tex.textureValue != null)
        {
            ...
            if (_DistortTexxx)
            {
                ...
                if (material.GetFloat("_IfFlowmap") == 0)
                {
                    ...
                    {
                        m_MaterialEditor.ShaderProperty(DistortRemap, "数据映射为-1到1");
                        if (_tips == true)
                        {
                            ...
                            style.fontSize = 10;
                            style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
                            GUILayout.Label("*把uv扭曲的数据从0到1映射到-1到1，使扭曲方向更加丰富", style);
                            EditorGUILayout.EndVertical();
                        }
            ...
            if (_tips == true)
            {
                ...
                style.fontSize = 10;
                style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
                GUILayout.Label("*开启自定义数据控制贴图扭曲程度，配合溶解可以用来制作flowmap溶解效果，开启自定义数据时请先添加uv2，再添加custom1.xyzw,再添加custom2.xyzw，然后custom2.w控制扭曲程度，开启自定义数据后，扭曲程度将会失效，并隐藏", style);
                EditorGUILayout.EndVertical();
            }
...

    {
        ...
        if (Vto_Tex.textureValue != null)
        {
            ...
            if (_tips == true)
            {
                ...
                style.fontSize = 10;
                style.normal.textColor = new Color(0.5f, 0.5f, 0.5f);
                GUILayout.Label("*开启自定义数据控制模型膨胀程度，可以制作爆炸之类的效果，开启自定义数据时请先添加uv2，再添加custom1.xyzw,再添加custom2.xyzw，然后custom1.w控制膨胀程度，开启自定义数据后，膨胀程度将会失效，并隐藏", style);
                EditorGUILayout.EndVertical();
            }
...

    {
        ...
        m_MaterialEditor.TexturePropertySingleLine(new GUIContent("附加贴图"), AddTex_Sampler, AddTexColor);
...

    {
        ...
        GUILayout.Space(5); GUILayout.Label(" 4.custom1.w控制顶点动画膨胀程度", style);
        GUILayout.Space(5); GUILayout.Label(" 5.custom2.xy控制遮罩图uv偏移", style);
        GUILayout.Space(5); GUILayout.Label(" 6.custom2.w控制uv扭曲强度", style);

        GUILayout.Space(5); GUILayout.Label(" 7.开启溶解拖尾模式后，透明度改为溶解参数", style);
