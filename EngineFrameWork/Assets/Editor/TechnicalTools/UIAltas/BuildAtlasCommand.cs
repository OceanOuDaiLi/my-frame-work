...
public class BuildAtlasCommand
{
    ...
    {
        get => IOHelper.IO.Disk(Application.dataPath + "/ABAssets/AssetBundle/ui");
    }
    private static IDirectory sourceSpriteDir
    {
        get => sourceSpriteDisk.Directory("prefabs");
    }
    ...
    private static IDisk sourceMapCommonSpriteDisk
    {
        get => IOHelper.IO.Disk(Application.dataPath + "/ArtAssets/mapCommon");//Assets/ABAssets/AssetBundle/pools/pool_map/bg_duihua_guangdian.png
    }
    ...
    public static void BuildUIAtlas()
    {
        ...
        for (int i = 0; i < directories.Length; i++)
        {
            ...
            tmpSourceDir = sourceSpriteDisk.Directory("prefabs/" + dirName + "/");
    ...
    private static void BuildUIAtlasFromDirectory(IDirectory directory, string dirName)
    {
        ...
        {
            if (tmpFile[i].Extension.Contains(".meta") || tmpFile[i].Extension.Contains(".spriteatlas"))
            {
                continue;
            }
            EditorUtility.DisplayProgressBar(string.Format($"Start Build {dirName} atlas .."), tmpFile[i].Name, i / tmpFile.Length);

            assetPath = "Assets/ABAssets/AssetBundle/ui/prefabs/" + dirName + "/" + tmpFile[i].Name;
    ...
    public static void BuildMapMaskAtlasFromDirectory(IDirectory directory, string dirName)
    {
        ...
        string dir = $"Assets/ABAssets/AssetBundle/map/base/{dirName}/";
        string filePath = dir + atlasName;
        if (!System.IO.Directory.Exists(dir))
        {
            System.IO.Directory.CreateDirectory(dir);
    ...
    public static void BuildMapCommonAtlas()
    {
        IDirectory rootDir = sourceMapCommonSpriteDisk.Root;
        IDirectory[] allDir = rootDir.ChildrenDirectorys;
        string rootPath = rootDir.Path.Replace("/", @"\") + @"\";
        foreach (IDirectory dir in allDir)
        {
            ...
            var dirName = rePath.Substring(rePath.LastIndexOf(@"\") + 1);
            Debug.Log(string.Format($" Building <color=#7BE578> {rePath} </color>  <color=#DEDD23> {dirName} </color>  atlas .."));
            BuildMapCommonAtlasFromDirectory(dir, dirName);
        }

        AssetDatabase.Refresh();
    }

    public static void BuildMapCommonAtlasFromDirectory(IDirectory directory, string dirName)
    {
        string atlasName = dirName + "_map_atlas" + ".spriteatlas";
        ...
        string dir = $"Assets/ABAssets/AssetBundle/pools/pool_map/";
        string filePath = dir + atlasName;
        if (!System.IO.Directory.Exists(dir))
        {
            System.IO.Directory.CreateDirectory(dir);
