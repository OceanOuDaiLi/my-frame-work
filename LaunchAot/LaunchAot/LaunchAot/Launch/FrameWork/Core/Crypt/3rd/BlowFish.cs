    ...
    class BlowFish
    {
        ...
        private byte[] Crypt_ECB(byte[] text, bool decrypt)
        {
            ...
            if (!decrypt)
            {
                ...
                int length = text.Length + padLen;
                byte[] plainText = new byte[length + 1];//预留一位存储补位长度
                ...
                //最后一个byte用来存储补位长度
                plainText[length] = (byte)padLen;
                return plainText;
            }
            else
            {
                int padLen = text[text.Length - 1];
                int length = text.Length - 1 - padLen;
                byte[] plainText = new byte[text.Length - 1];
                Buffer.BlockCopy(text, 0, plainText, 0, text.Length - 1);
                ...
                byte[] trims = new byte[length];
                Buffer.BlockCopy(plainText, 0, trims, 0, length);
                return trims;
            }
        }
        ...
        {
            xl_par = (uint)((block[0] << 24) | (block[1] << 16) | (block[2] << 8) | block[3]);
            xr_par = (uint)((block[4] << 24) | (block[5] << 16) | (block[6] << 8) | block[7]);
        ...
        {
            block[0] = (byte)(xl_par >> 24);
            block[1] = (byte)(xl_par >> 16);
            block[2] = (byte)(xl_par >> 8);
            block[3] = (byte)(xl_par);
            block[4] = (byte)(xr_par >> 24);
            block[5] = (byte)(xr_par >> 16);
            block[6] = (byte)(xr_par >> 8);
            block[7] = (byte)(xr_par);
