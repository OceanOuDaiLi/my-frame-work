    ...
    public sealed class KService : AService
    {
        ...
        private static void KcpLog(IntPtr bytes, int len, IntPtr kcp, IntPtr user)
        {
            try
            {
                ...
                SDebug.Info(logBuffer.ToStr(0, len));
            }
            catch (Exception e)
            {
                SDebug.LogError(e);
        ...
        public void ChangeAddress(long id, IPEndPoint address)
        {
            ...
            SDebug.Info($"channel change address: {id} {address}");
                    ...
                    switch (flag)
                    {
                        ...
                        case KcpProtocalType.SYN: // accept
                        {
                            ...
                            try
                            {
                                ...
                                SDebug.Info($"kservice syn: {kChannel.Id} {remoteConn} {localConn}");
                        ...
                        case KcpProtocalType.ACK: // connect返回
                            ...
                            if (kChannel != null)
                            {
                                SDebug.Info($"kservice ack: {kChannel.Id} {remoteConn} {localConn}");
                        ...
                        case KcpProtocalType.FIN: // 断开
                            ...
                            SDebug.Info($"kservice recv fin: {kChannel.Id} {localConn} {remoteConn} {error}");
        ...
        public override void Remove(long id)
        {
            ...
            SDebug.Info($"kservice remove channel: {id} {kChannel.LocalConn} {kChannel.RemoteConn}");
        ...
        private void Disconnect(uint localConn, uint remoteConn, int error, IPEndPoint address, int times)
        {
            ...
            SDebug.Info($"channel send fin: {localConn} {remoteConn} {address} {error}");
        }
