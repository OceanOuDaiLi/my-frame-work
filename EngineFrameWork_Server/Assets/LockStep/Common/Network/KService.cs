    ...
    public sealed class KService : AService
    {
        ...
        private static void KcpLog(IntPtr bytes, int len, IntPtr kcp, IntPtr user)
        {
            try
            {
                ...
                EDebug.Log(logBuffer.ToStr(0, len));
            }
            catch (Exception e)
            {
                EDebug.LogError(e);
        ...
        public void ChangeAddress(long id, IPEndPoint address)
        {
            ...
            EDebug.Log($"channel change address: {id} {address}");
        ...
        private void Recv()
        {
            ...
            while (socket != null && this.socket.Available > 0)
            {
                ...
                try
                {
                    ...
                    switch (flag)
                    {
                        ...
                        case KcpProtocalType.SYN: // accept
                        {
                            ...
                            {
                                EDebug.LogError($"kchannel syn address diff: {kChannel.Id} {kChannel.RealAddress} {realAddress}");
                                break;
                            }
                            ...
                            try
                            {
                                ...
                                EDebug.Log($"kservice syn: {kChannel.Id} {remoteConn} {localConn}");
                        ...
                        case KcpProtocalType.ACK: // connect返回
                            ...
                            if (kChannel != null)
                            {
                                EDebug.Log($"kservice ack: {kChannel.Id} {remoteConn} {localConn}");
                        ...
                        case KcpProtocalType.FIN: // 断开
                            ...
                            EDebug.Log($"kservice recv fin: {kChannel.Id} {localConn} {remoteConn} {error}");
                ...
                {
                    EDebug.LogError($"kservice error: {flag} {remoteConn} {localConn}\n{e}");
                }
            }
        }
        ...
        protected override void Get(long id, IPEndPoint address)
        {
            ...
            {
                EDebug.LogError($"kservice get error: {id}\n{e}");
            }
        }
        ...
        public override void Remove(long id)
        {
            ...
            EDebug.Log($"kservice remove channel: {id} {kChannel.LocalConn} {kChannel.RemoteConn}");
        ...
        private void Disconnect(uint localConn, uint remoteConn, int error, IPEndPoint address, int times)
        {
            ...
            {
                EDebug.LogError($"Disconnect error {localConn} {remoteConn} {error} {address} {e}");
            }

            EDebug.Log($"channel send fin: {localConn} {remoteConn} {address} {error}");
        }
        ...
        private void RemoveConnectTimeoutChannels()
        {
            ...
            foreach (long channelId in this.waitConnectChannels.Keys)
            {
                ...
                if (kChannel == null)
                {
                    EDebug.LogError($"RemoveConnectTimeoutChannels not found kchannel: {channelId}");
                    continue;
                }
