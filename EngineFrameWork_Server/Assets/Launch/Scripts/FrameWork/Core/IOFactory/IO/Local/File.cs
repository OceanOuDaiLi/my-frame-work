...
namespace Core.IO
{
    ...
    public sealed class File : IFile
    {
        ...
        public void Create(byte[] data)
        {
            ...
            if (data.Length > 0) data = Encrypted(data);
        ...
        public void Append(byte[] data)
        {
            ...
            data = Encrypted(data);
            using (var fs = new FileStream(FullName, System.IO.FileMode.Append, FileAccess.Write))
            {
                fs.Write(data, 0, data.Length);
                fs.Close();
                fs.Dispose();
        ...
        public void AppendAsync(byte[] data, Action callback)
        {
            if (Exists)
            {
                throw new IOException("file is exists");
        ...
        private byte[] Encrypted(byte[] data)
        {
            ...
            {
                data = disk.IOCrypt.Encrypted(data);
            }
            return data;
        }
        ...
        private byte[] Decrypted(byte[] data)
        {
            if (disk.IOCrypt != null)
            {
                data = disk.IOCrypt.Decrypted(data);
            }
            return data;
        }
