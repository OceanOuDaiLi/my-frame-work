...
namespace Core.IO
{
    ...
    public sealed class Directory : IDirectory
    {
        ...
        /// 返回此目录的所有子目录
        /// </summary>
        public IDirectory[] ChildrenDirectorys
        {
            get
            {
                var infos = DirectoryInfo.GetDirectories("*", SearchOption.AllDirectories);
                IDirectory[] result = new IDirectory[infos.Length];
                IDirectory tmpDir = null;
                for (int i = 0; i < infos.Length; i++)
                {
                    tmpDir = new Directory(infos[i].FullName, disk);
                    result[i] = tmpDir;
                }
        ...
        public IDirectory CopyTo(string targetDirectory)
        {
            LocalDisk.GuardValidatePath(targetDirectory);
            IDirectory dir = new Directory(targetDirectory, disk);
            dir.Create();
            ...
            foreach (var s in files)
            {
                ...
                destFile = targetDirectory + System.IO.Path.AltDirectorySeparatorChar + fileName;
            ...
            foreach (var info in System.IO.Directory.GetDirectories(Path))
            {
                ...
                drinfo.CopyTo(targetDirectory + System.IO.Path.AltDirectorySeparatorChar + drinfo.Name);
            }
