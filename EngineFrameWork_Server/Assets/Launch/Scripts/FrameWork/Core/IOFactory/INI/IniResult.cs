...
namespace Core.INI
{
    ...
    public sealed class IniResult : IIniResult
    {
        ...
        {
            if (!iniDict.ContainsKey(section)) return new Dictionary<string, string>();
            return iniDict[section];
        }
        ...
        /// <param name="kv"></param>
        public void Set(string section, Dictionary<string, string> kv)
        {
            if (kv.Count == 0) return;
            if (!iniDict.ContainsKey(section))
            {
                iniDict.Add(section, new Dictionary<string, string>(kv));
            }
            else
            {
                iniDict[section] = new Dictionary<string, string>(kv);
            }
        }
        ...
        {
            var fileString = data == null ? "" : Encoding.UTF8.GetString(data);
