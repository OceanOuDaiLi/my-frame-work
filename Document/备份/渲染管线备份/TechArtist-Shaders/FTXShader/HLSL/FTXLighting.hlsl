...
half3 FTX_LightingPhysicallyBased_SKin(BRDFData brdfData, Light light, half3 normalWS, half3 viewDirectionWS, half thickness, half occlusion, bool specularHighlightsOff)
{
    half NdotL = saturate(dot(normalWS, light.direction));
    half3 radiance = light.color * light.distanceAttenuation * light.shadowAttenuation;// 没有 * NdotL，下面补上了
    
    half3 brdf = brdfData.diffuse * CalculateSSSTerm(NdotL);
#ifndef _SPECULARHIGHLIGHTS_OFF
    [branch] if (!specularHighlightsOff)
    {
        brdf += brdfData.specular * DirectBRDFSpecular(brdfData, normalWS, light.direction, viewDirectionWS)  * NdotL;
    }
...
half4 FTX_UniversalFragmentPBR_Skin(InputData inputData, SkinSurfaceData skinSurfaceData)
{
    ...
    color += FTX_LightingPhysicallyBased_SKin(brdfData, mainLight,
                                              inputData.normalWS, inputData.viewDirectionWS,
                                              skinSurfaceData.thickness, skinSurfaceData.occlusion,
...
#ifdef UNIVERSAL_PLAYER_INPUT_INCLUDED
half3 FTX_LightingPhysicallyBased(BRDFData brdfData, half3 reflection, half3 normalWS, half3 viewDirectionWS, bool specularHighlightsOff)
{
    half3 radiance = reflection;
