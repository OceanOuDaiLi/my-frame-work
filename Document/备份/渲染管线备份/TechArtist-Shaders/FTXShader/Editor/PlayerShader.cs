...
{
    internal class PlayerShader : BaseShaderGUI
    {
        ...
        public static void DoMetallicSpecularArea(FTXLitProperties properties, MaterialEditor materialEditor, Material material)
        {
                ...
                (WorkflowMode)properties.workflowMode.floatValue == WorkflowMode.Metallic)
            {
                ...
                GUILayout.Label("         (G : AO   A  : Smoothness )");
        ...
        {
            if (litProperties.playerReflections != null && litProperties.highlights != null)
            {
                EditorGUI.BeginChangeCheck();
                materialEditor.ShaderProperty(litProperties.highlights, LitGUI.Styles.highlightsText);
                materialEditor.ShaderProperty(litProperties.playerReflections, new GUIContent("Player Reflection Light"));

                if(litProperties.playerReflections.floatValue == 1)
                {
                    material.EnableKeyword("PLAYERREFLECTION");
                }
                else
                {
                    material.DisableKeyword("PLAYERREFLECTION");
                }
