...
namespace VLB
{
    ...
    public class BeamGeometry : MonoBehaviour
    {
        ...
        public void UpdateMaterialAndBounds()
        {
            ...
            //这里没有深度图是没有效果的，另外这个条件判断不对，shader中采样的深度图名字也不对(当然我也不想它开启)，需要改造成URP版本来开启深度图或判断是否有深度图
            if (m_Master.depthBlendDistance > 0f && UnityEngine.Rendering.Universal.UniversalRenderPipeline.asset.supportsCameraDepthTexture)
            {
