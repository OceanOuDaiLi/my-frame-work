...
namespace AI.Tools
{
    public class BaseBrush : EditorWindow
    {
        ...
        // Tips
        private const string _headTips = "头部设置";
        private const string _clothTips = "上身设置";
        private const string _pantsTips = "四肢设置";
        private const string _shoesTips = "鞋子设置";


        // Data
        public GameObject _curObj;
        public GameObject _headObj;
        public GameObject _clothObj;
        public GameObject _limbsObj;
        public GameObject _shoesObj;
        public GameObject _colorObj;

        public SkinnedMeshRenderer headRender;
        public SkinnedMeshRenderer clothRender;
        public SkinnedMeshRenderer limbsRender;
        public SkinnedMeshRenderer shoesRender;
        ...
        public void DrawHead()
        {
            ...
            var c = GUI.color;
            GUILayout.Label("FBXResource", _objFieldLableStyle);
            GUILayout.BeginHorizontal();
            _current.heads.list[0].oriObj = (GameObject)EditorGUILayout.ObjectField(_current.heads.list[0].oriObj, typeof(GameObject), true);
            GUI.color = c * Color.green;
            if (GUILayout.Button(ToStr(true), _badgeStyle, GUILayout.MaxWidth(20.0f))) { }
            GUILayout.EndHorizontal();

            GUI.color = c;


            GUILayout.Label("NormalMap", _objFieldLableStyle);
            if (_current.heads.headNormal == null)
            {
                _current.heads.LoadHeadNormal(_current.DEFAULT_HEAD_NORMAL_TEX_PATH);
            }
            _current.heads.headNormal = (Texture2D)EditorGUILayout.ObjectField(_current.heads.headNormal, typeof(Texture2D), true);
        ...
        {
            _flodOutIdx = 1;
            DrawToggleItem(_clothTips);
            if (!_isClothFlodout)
            {
                return;
            }

            DrawObjectFieldItem(_current.cloth.list, _current.cloth.curLevel, _current.cloth.selectLevel, BrushModelType.CLOTH);


            GUILayout.Label("NormalMap", _objFieldLableStyle);
            if (_current.cloth.clothNormal == null)
            {
                _current.cloth.LoadClothNornal(_current.DEFAULT_CLOTH_NORMAL_TEX_PATH);
            }
            _current.cloth.clothNormal = (Texture2D)EditorGUILayout.ObjectField(_current.cloth.clothNormal, typeof(Texture2D), true);
        ...
        {
            _flodOutIdx = 2;
            DrawToggleItem(_pantsTips);
            if (!_isPantsFlodout)
            {
                return;
            }

            DrawObjectFieldItem(_current.limbs.list, _current.limbs.curLevel, _current.limbs.selectLevel, BrushModelType.LIMBS);

            GUILayout.Label("NormalMap", _objFieldLableStyle);
            if (_current.limbs.limbNormal == null)
            {
                _current.limbs.LoadLimbNornal(_current.DEFAULT_LIMBS_NORMAL_TEX_PATH);
            }
            _current.limbs.limbNormal = (Texture2D)EditorGUILayout.ObjectField(_current.limbs.limbNormal, typeof(Texture2D), true);
        ...
        {
            _flodOutIdx = 3;
            DrawToggleItem(_shoesTips);
            if (!_isShoesFlodout)
            {
                return;
            }

            DrawObjectFieldItem(_current.shoes.list, _current.shoes.curLevel, _current.shoes.selectLevel, BrushModelType.SHOES);

            GUILayout.Label("NormalMap", _objFieldLableStyle);
            if (_current.shoes.shoesNormal == null)
            {
                _current.shoes.LoadShoesNornal(_current.DEFAULT_SHOES_NORMAL_TEX_PATH);
            }
            _current.shoes.shoesNormal = (Texture2D)EditorGUILayout.ObjectField(_current.shoes.shoesNormal, typeof(Texture2D), true);
        ...
        public void DrawAutoSetModelLevel()
        {
            string caption = "一键选择高低模";
            GUIStyle style = new GUIStyle(EditorStyles.toolbarButton);
            style.alignment = TextAnchor.MiddleLeft;
            style.fontSize = 14;
            ...
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            {
                if (GUILayout.Button("一键选择高低模", _badgeStyle))
                {
                    _current.SetModelLevel(BrushModelLevel.LOW);
                }
            }
            EditorGUILayout.EndHorizontal();
            ...
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            {
                if (GUILayout.Button("一键选择高模", _badgeStyle))
                {
                    _current.SetModelLevel(BrushModelLevel.HIGH);
                }
            }
            EditorGUILayout.EndHorizontal();
