...
public class CompressAtlas : EditorWindow
{
    public enum MaxSize
    {
        LESS_256 = 0,
    ...
    public static CompressAtlasConfigSO config;
    ...
    public static bool foldout1 = true;
    public static bool foldout2 = true;
    //白名单
    ...
    //和合法性检测
    static string illegalFileLogPath = "Log/";
    ...
    public static List<Queue<string>> illegalDebugLogQueue = new List<Queue<string>>() { new Queue<string>(), new Queue<string>(), new Queue<string>() };
    ...
    static string configSOPath = "Assets/Editor/SO/CompressAtlasSO.asset";

    [MenuItem("公共工具/TATools/图片MaxSize压缩工具")]
    static void Init()
    {
        GetWindow(typeof(CompressAtlas));
    }
    ...
    private void OnGUI()
    {
        ...
        else if (compressToolBarIndex == 0)
        {
            ...
            EditorGUILayout.LabelField("不合法文件Excel日志路径", "项目根目录/" + illegalFileLogPath);
            GUILayout.Space(10);
            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button("保存配置", GUILayout.Height(25)))
            {
        ...
        string toggle1Tips = "当前选择文件夹目录: (请在Project面板拖拽文件夹目录进来) ";
        EditorGUILayout.BeginHorizontal();
        foldout1 = GUILayout.Toggle(foldout1, toggle1Tips, toggle1Style);
        ...
        //文件夹列表
        string toggle2Tips = "白名单文件/文件夹目录: (请在Project面板拖拽文件夹目录进来)";
        EditorGUILayout.BeginHorizontal();
        foldout2 = GUILayout.Toggle(foldout2, toggle2Tips, toggle1Style);
        EditorGUILayout.LabelField($"{whiteListFileObjs.Count}", GUILayout.Width(25));
        EditorGUILayout.EndHorizontal();
        if (foldout2)
        {
    ...
    {
        var to = AssetDatabase.LoadAssetAtPath<CompressAtlasConfigSO>(configSOPath);
        if (to == null) return;
        to.level = level;
        ...
        AssetDatabase.SaveAssets();
        Debug.Log($"[图片压缩工具]: 保存配置成功");
    }
    static void LoadConfigFromSO()
    {
        config = AssetDatabase.LoadAssetAtPath<CompressAtlasConfigSO>(configSOPath);
    ...
    {
        if (illegalAlphafilePaths.Count > 0)
        {
        ...
        if (illegalWidthHeightfilePaths_Android.Count > 0)
        {
        ...
        if (illegalWidthHeightfilePaths_IOS.Count > 0)
        {
        ...
        WriteLogToExcelFile(ref illegalDebugLogQueue);
    ...
    static void WriteLogToExcelFile(ref List<Queue<string>> logList)
    {
        logList[0].Enqueue("[Alpha] 没有Alpha的文件");
        for (int i = illegalAlphafilePaths.Count - 1; i < illegalAlphafilePaths.Count && i >= 0; i--)
        {
            logList[0].Enqueue(illegalAlphafilePaths[i]);
        }
        logList[1].Enqueue($"[Android] 图片宽高不合法文件 不是2的n次幂");
        for (int i = illegalWidthHeightfilePaths_IOS.Count - 1; i < illegalWidthHeightfilePaths_IOS.Count && i >= 0; i--)
        {
            logList[1].Enqueue(illegalWidthHeightfilePaths_IOS[i]);
        }
        logList[2].Enqueue($"[IOS] 图片宽高不合法文件 不是4的倍数");
        for (int i = illegalWidthHeightfilePaths_Android.Count - 1; i < illegalWidthHeightfilePaths_Android.Count && i >= 0; i--)
        {
            logList[2].Enqueue(illegalWidthHeightfilePaths_Android[i]);
        }
        var time = $"[{System.DateTime.Now.Hour}h{System.DateTime.Now.Minute}m{System.DateTime.Now.Second}s]";
        var fileName = $"{illegalFileLogPath}illigalImages_{System.DateTime.Now.Year}-{System.DateTime.Now.Month}-{System.DateTime.Now.Day}-{time}.xlsx";
        FileStream fileStream = new FileStream(fileName, FileMode.OpenOrCreate);
        StreamWriter sw = new StreamWriter(fileStream);

        while (logList[0].Count > 0 || logList[1].Count > 0 || logList[2].Count > 0)
        {
            foreach (var queue in logList)
            {
                if (queue.Count > 0)
                {
        ...
        Debug.Log($"[不合法文件记录成功] excel路径：{fileName}");
    }
    ...
    {
        //android
        SetTextureFormatForAllSize(CompressPlatform.ANDROID, CompressLevel.LOW, HasAlpha.NOALPHA, TextureImporterFormat.ETC_RGB4, TextureImporterFormat.ETC_RGB4, TextureImporterFormat.ETC_RGB4, TextureImporterFormat.ETC_RGB4, TextureImporterFormat.ETC_RGB4);
        SetTextureFormatForAllSize(CompressPlatform.ANDROID, CompressLevel.LOW, HasAlpha.HASALPHA, TextureImporterFormat.ETC2_RGB4, TextureImporterFormat.ETC2_RGB4, TextureImporterFormat.ETC2_RGBA8, TextureImporterFormat.ETC2_RGBA8, TextureImporterFormat.ETC2_RGBA8);
        SetTextureFormatForAllSize(CompressPlatform.ANDROID, CompressLevel.MEDIUM, HasAlpha.NOALPHA, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_4x4);
        SetTextureFormatForAllSize(CompressPlatform.ANDROID, CompressLevel.MEDIUM, HasAlpha.HASALPHA, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_4x4);
        SetTextureFormatForAllSize(CompressPlatform.ANDROID, CompressLevel.HIGH, HasAlpha.NOALPHA, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_4x4);
        SetTextureFormatForAllSize(CompressPlatform.ANDROID, CompressLevel.HIGH, HasAlpha.HASALPHA, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_4x4);
        //iphone
        SetTextureFormatForAllSize(CompressPlatform.IPHONE, CompressLevel.LOW, HasAlpha.NOALPHA, TextureImporterFormat.ASTC_8x8, TextureImporterFormat.ASTC_8x8, TextureImporterFormat.ASTC_8x8, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6);
        SetTextureFormatForAllSize(CompressPlatform.IPHONE, CompressLevel.LOW, HasAlpha.HASALPHA, TextureImporterFormat.ASTC_8x8, TextureImporterFormat.ASTC_8x8, TextureImporterFormat.ASTC_8x8, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6);
        SetTextureFormatForAllSize(CompressPlatform.IPHONE, CompressLevel.MEDIUM, HasAlpha.NOALPHA, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_4x4);
        SetTextureFormatForAllSize(CompressPlatform.IPHONE, CompressLevel.MEDIUM, HasAlpha.HASALPHA, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_4x4);
        SetTextureFormatForAllSize(CompressPlatform.IPHONE, CompressLevel.HIGH, HasAlpha.NOALPHA, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_4x4);
        SetTextureFormatForAllSize(CompressPlatform.IPHONE, CompressLevel.HIGH, HasAlpha.HASALPHA, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_4x4);
    }
    ...
    private static MaxSize GetMaxSizeEnum(int size)
    {
        ...
        if (size < 256)
            ...
            maxSize = MaxSize.LESS_2048;
        else if (size >= 2048)
            maxSize = MaxSize.GREATER_EQUAL_2048;
        return maxSize;
    }
