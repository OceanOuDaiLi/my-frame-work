...
public class ExcelImporterMaker : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        sepalateSheet = EditorGUILayout.Toggle("拆分 Sheet", sepalateSheet);
        ...
        foreach (ExcelSheetParameter sheet in sheetList)
        {
            if (firstEnter)
            {
                sheet.isEnable = idx == 0;
                idx++;
            }
            if (idx >= sheetList.Count)
            {
                firstEnter = false;
            }
    ...
    private bool sepalateSheet = true;
    ...
    [MenuItem("Assets/Create/程序工具/Excel(.xlsx) To ScriptableObject")]
    static void ExportExcelToAssetbundle()
    {
        foreach (Object obj in Selection.objects)
        {
            ...
            using (FileStream stream = File.Open(window.filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                ...
                window.className = EditorPrefs.GetString(s_key_prefix + window.fileName + ".className", window.fileName);
    ...
    void ExportEntity()
    {
        string templateFilePath = (sepalateSheet) ? "Assets/Editor/TechnicalTools/Terasurware/EntityTemplate2.txt" : "Assets/Editor/TechnicalTools/Terasurware/EntityTemplate.txt";
        ...
        if (!Directory.Exists("Assets/Scripts/Terasurware/"))
        {
            Directory.CreateDirectory("Assets/Scripts/Terasurware/");
        }
        File.WriteAllText("Assets/Scripts/Terasurware/" + className + ".cs", entittyTemplate);
    }

    void ExportImporter()
    {
        string templateFilePath = (sepalateSheet) ? "Assets/Editor/TechnicalTools/Terasurware/ExportTemplate2.txt" : "Assets/Editor/TechnicalTools/Terasurware/ExportTemplate.txt";
        ...
        importerTemplate = importerTemplate.Replace("$ExportAssetDirectry$", @"Assets/ABAssets/AssetBundle/terasurware/"/* Path.GetDirectoryName(filePath)*/);
        ...
        Directory.CreateDirectory("Assets/Scripts/Terasurware/Editor/");
        File.WriteAllText("Assets/Scripts/Terasurware/Editor/" + fileName + "_importer.cs", importerTemplate);
